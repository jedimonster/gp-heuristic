...
/**
  ...
  */
class RandomGrowInitializer(params: List[Any], val methodCount: Int, prototypeFile: File) extends PopulationInitializer[JavaCodeIndividual] {
  ...
  override def getInitialPopulation(n: Int): List[JavaCodeIndividual] = {
    // todo note commented it out, why is it here?
...
//    }
  ...
  def growIndividual(id: Int): JavaCodeIndividual = {
    ...
    } // todo set proper type argugments for the gpprogram?
    // change its name
    individual.setName("Ind" + id.toString)
    individual.gardener = Some(this)

    // randomly grow methods
    val methods: IndexedSeq[MethodDeclaration] = for (i <- 0 until methodCount) yield growMethod(HeuristicsNumbers.getNext, ParamCount, individual)
  ...
  def growMethod(id: Int, paramCount: Int, individual: JavaCodeIndividual): MethodDeclaration = {
    ...
    if (Math.random() > 0.5) { // either grow a for loop or a regular statement
      ...
      var nodesToReturn = expandedParams.filter(can => can.referenceType.toString.equals("double") || can.referenceType.toString.equals("int"))
